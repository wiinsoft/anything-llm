...
export default function GenericOpenAiOptions({ settings }) {
  ...
  const [apiKey, setApiKey] = useState(settings?.GenericOpenAiKey);
  const [basePathValue, setBasePathValue] = useState(settings?.GenericOpenAiBasePath);
      ...
      <div className="flex gap-[36px] mt-1.5 flex-wrap">
        <div className="flex flex-col w-60">
          ...
          <input
            ...
            placeholder="eg: https://total.wiinsoft.com/v1"
        ...
        <div className="flex flex-col w-60">
          ...
          <input
            ...
            className="border-none bg-theme-settings-input-bg text-white placeholder:text-theme-settings-input-placeholder text-sm rounded-lg focus:outline-primary-button active:outline-primary-button outline-none block w-full p-2.5"
            placeholder="Your WiinsAi API Key"
        ...
        {!settings?.credentialsOnly && (
          <ModelSelector settings={settings} apiKey={apiKey} basePath={basePath} />
        )}
      </div>
...
function ModelSelector({ settings, apiKey, basePath }) {
  ...
  useEffect(() => {
    async function fetchModels() {
      ...
      setLoading(true);
      const { models: fetched = [] } = await System.customModels(
        "generic-openai",
        null,
        basePath
      );
