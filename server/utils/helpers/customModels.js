...
async function genericOpenAiModels(basePath = null, apiKey = null) {
  if (!basePath) basePath = process.env.GENERIC_OPEN_AI_BASE_PATH;
  if (!apiKey) apiKey = process.env.GENERIC_OPEN_AI_API_KEY;
  if (!basePath) return { models: [], error: null };
  try {
    const resp = await fetch(basePath.replace(/\/+$/, '') + '/models', {
      method: 'GET',
      headers: {
